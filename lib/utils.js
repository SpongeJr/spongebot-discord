...
module.exports = {
	...
	addNick: function(who, nick, gameStats) {
		...
		if (!gameStats[who]) {
			...
			console.log('Just set gameStats.' + who + '.profile.nick to ' + nick);
			gameStats[who].profile.nick = nick;
			return;
		}
	},
	setStat: function(who, game, stat, val, gameStats) {
		var nick = 'some user'; // default if we don't have a nick
		...
		if (typeof who === 'string') {
			...
			nick = who.username;
			who = who.id;
			console.log('nick is ' + nick + ' and who.id is ' + who.id);
		}
	...
	alterStat: function(who, game, stat, amt, gameStats) {
		...
		if (typeof who === 'string') {
			...
			nick = who.username;
			who = who.id;
			console.log('nick is ' + nick + ' and who ' + who);
		}
