...
module.exports = {
	...
	collectTimer: function(message, who, timerName, botCommand, gameStats) {
		// who: (String) an id, or a tag (will be sent through utils.makeId() )
		// botCommand: the spongeBot.yourCommand passed in
		// timerName: the command name and timer name linked to it
		...
		var now = new Date();
		var timedCmd = botCommand.timedCmd;
	...
	checkTimer: function(message, who, timerName, botCommand, gameStats) {
		// who: (String) an id, or a tag (will be sent through utils.makeId() )
		// command: spongeBot.yourCommand passed in
